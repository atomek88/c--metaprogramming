...
public:
  ...
  T get() {
    unique_lock<mutex> lck{sharedState->mtx};
    sharedState->cv.wait(lck, [&] {return sharedState->state != SharedState<T>::empty; });
    switch (sharedState->state) {
    case SharedState<T>::val:
      return move(*sharedState->value);
    case SharedState<T>::exc:
...
private:
  ...
  MyFuture(shared_ptr<SharedState<T>> &sharedState) : sharedState(sharedState) {}
...
public:
  ...
  void set_value(const T &value) {
    lock_guard<mutex> lck(sharedState->mtx);
    sharedState->value = make_unique<T>(value);
    sharedState->state = SharedState<T>::val;
  ...
  void set_exception(exception_ptr exc) {
    ...
    sharedState->state = SharedState<T>::exc;
    sharedState->cv.notify_one();
  }
